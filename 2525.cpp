...
int main()
{
	...
	do
	{
		printf("请输入要计算的普通管数据的组数（1-5）,如果希望退出，输入0:>");
		scanf("%d", &i);
		...
		tJ[0] = (aJ[0] + aC[0]) / 2;
		printf("参考平均温度为：%lf，", tJ[0]);
		printf("请查表输入近似温度下空气的密度、定压比热容、粘度、导热系数\n");
		printf("粘度和导热系数输入时，只需要输入前部分，不需输入次方项\n");
		scanf("%lf%lf%lf%lf", &rou, &cp, &u, &n);
		for (j = 0; j < i; j++)
		{
