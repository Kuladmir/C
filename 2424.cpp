...
int main()
{
	...
	printf("这是本程序默认采取的常数：\n");
	printf("Π = %.2lf   压强表内径D = %.3lfm  真空表内径d = %.3lfm   水的密度ρ= %dkg/m^3   高度差h = %.2lfm   重力加速度g = %.2lfN/kg\n", PI, D, d, Rou, h, g);
	printf("如需修改，请联系管理员\n");
	...
	do
	{
		...
		if (i != 0)
		{
			printf("你希望计算第一部分数据，还是第二部分，前者请输入1，后者请输入2\n");
			scanf("%d", &cho);
			if (cho == 2)
			{
				printf("请依次输入 %d 组 P出 的数据:>", i);
				for (j = 0; j < i; j++)
					scanf("%lf", &a[j]);//接收p出
				printf("请依次输入 %d 组 P入 的数据:>", i);
				for (j = 0; j < i; j++)
					scanf("%lf", &b[j]);//接收p入
				printf("请依次输入 %d 组的流量Q:>", i);
				for (j = 0; j < i; j++)
					scanf("%lf", &c[j]);//接收流量Q
				for (j = 0; j < i; j++)
				{
